...
- **Phase 5-8**: mem, ps, ip, run, spawn, dns, http, halt をユーザー空間に移行
- **カーネル起動時にユーザーシェルを自動起動**: カーネルシェルを廃止してユーザーシェルがデフォルトになるようにする
- **selftest のユーザー空間移行**: selftest コマンドもユーザー空間で動かす
