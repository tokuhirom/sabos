...
### 完了: ユーザーランド FAT32 直接アクセスの廃止

セキュリティ上の大きな問題を解決した。これまでユーザーランドのプログラム（shell.rs, term.rs, ed.rs）が `user/src/fat32.rs` を通じて `SYS_BLOCK_READ` / `SYS_BLOCK_WRITE` syscall でディスクに直接アクセスし、FAT32 ファイルシステム操作を行っていた。

これはユーザー空間のプログラムがブロックデバイスの任意のセクタを読み書きできてしまうということで、例えば悪意あるプログラムが他のファイルのデータを直接読んだり、FAT テーブルを壊してファイルシステムを破壊したりできる。マイクロカーネル移行のロードマップにも反する設計だった。

#### 新しい syscall（14〜17番）

ファイルシステム操作をすべてカーネル経由にするため、4 つの新しいシステムコールを追加した:

| 番号 | 名前 | 引数 | 用途 |
|------|------|------|------|
| 14 | SYS_FILE_WRITE | (path_ptr, path_len, data_ptr, data_len) | ファイル作成/上書き |
| 15 | SYS_DIR_CREATE | (path_ptr, path_len) | ディレクトリ作成 |
| 16 | SYS_DIR_REMOVE | (path_ptr, path_len) | ディレクトリ削除 |
| 17 | SYS_FS_STAT | (buf_ptr, buf_len) | ファイルシステム統計（JSON） |

既存の `SYS_FILE_DELETE(12)` / `SYS_DIR_LIST(13)` と合わせて、ファイルシステムの CRUD 操作がすべてパスベース syscall で完結するようになった。

#### 移行の手順

1. カーネル FAT32 に `cluster_bytes()` メソッドを追加
2. カーネル `syscall.rs` に 4 つの新 syscall ハンドラを実装
3. ユーザー `syscall.rs` に `file_write()`, `dir_create()`, `dir_remove()`, `fs_stat()` ラッパーを追加
4. `user/src/bin/shell.rs` の write, rm, mkdir, rmdir, df コマンドを syscall 経由に書き換え
5. `user/src/bin/term.rs` の write, rm コマンドを syscall 経由に書き換え
6. `user/src/bin/ed.rs` のファイル保存を syscall 経由に書き換え
7. `user/src/shell.rs`（ライブラリ版シェル）も同様に書き換え
8. `user/src/fat32.rs` と `user/src/fat16.rs` を削除
9. selftest に `syscall_file_write`, `syscall_dir_ops`, `syscall_fs_stat` を追加

#### 設計のポイント

- **SYS_FILE_WRITE** は既存ファイルがあれば自動で削除してから作成する（ユーザー側 fat32.rs の `create_file` と同じ挙動）
- すべての書き込み系 syscall で `/proc` 配下は `ReadOnly` エラーを返す（procfs は表示専用の設計方針）
- **SYS_FS_STAT** は JSON 形式で統計情報を返す。df コマンドがカーネルから直接 JSON を受け取れるので、ユーザー側で数値をフォーマットする必要がなくなった

#### 変更ファイル

| ファイル | 変更内容 |
|---------|---------|
| `kernel/src/fat32.rs` | `cluster_bytes()` メソッド追加 |
| `kernel/src/syscall.rs` | SYS_FILE_WRITE(14), SYS_DIR_CREATE(15), SYS_DIR_REMOVE(16), SYS_FS_STAT(17) |
| `kernel/src/shell.rs` | selftest に 3 テスト追加 |
| `user/src/syscall.rs` | `file_write()`, `dir_create()`, `dir_remove()`, `fs_stat()` ラッパー |
| `user/src/bin/shell.rs` | fat32 直接アクセスを syscall に置換、`mod fat32` 削除 |
| `user/src/bin/term.rs` | fat32 直接アクセスを syscall に置換、`mod fat32` 削除 |
| `user/src/bin/ed.rs` | fat32 直接アクセスを syscall に置換、`mod fat32` 削除 |
| `user/src/shell.rs` | fat32 直接アクセスを syscall に置換 |
| `user/src/main.rs` | `mod fat32` 削除 |
| `user/src/fat32.rs` | **削除** |
| `user/src/fat16.rs` | **削除**（未使用） |
| `docs/spec/syscall-list.md` | 新 syscall の記載 |

selftest **35/35 PASS**。

## フレームバッファのバックバッファ実装

GUI がもっさりしていることに気づいた。原因を調査したところ、**カーネルの `put_pixel` が MMIO（フレームバッファ）に直接 `write_volatile` していた**のが最大のボトルネックだった。

MMIO への書き込みは通常の RAM アクセスの 100〜1000 倍遅い。例えば 1024×768 の画面で `draw_blit` を呼ぶと、約 78 万回の `write_volatile` が発生する。1 回が 100ns としても約 78ms かかり、フレーム落ちの原因になる。スクロール処理では MMIO 領域間の `core::ptr::copy`（数 MB の memmove）が走っていて、これも非常に遅い。

### 解決策: RAM 上のバックバッファ

`FramebufferWriter` に `Vec<u8>` のバックバッファを追加した。描画の流れ:

1. `put_pixel` はバックバッファ（RAM）に書き込む → 通常のメモリアクセスなので高速
2. 描画処理の最後に `flush()` または `flush_rect()` で MMIO に一括転送
3. `core::ptr::copy_nonoverlapping` による一括コピーは、ピクセルごとの `write_volatile` より桁違いに速い

### 変更点

| 処理 | Before | After |
|------|--------|-------|
| `put_pixel` | `write_volatile` で MMIO 直接 | `backbuf[offset]` に RAM 書き込み |
| `clear()` | ピクセルループで MMIO | `chunks_exact_mut` でバックバッファ一括塗り |
| `scroll_up()` | MMIO 間 `core::ptr::copy` | `backbuf.copy_within`（RAM 内 memmove） |
| `draw_blit_global()` | put_pixel×78万回 | バックバッファ直接書き込み + `flush_rect` |
| `_print()` | 文字ごとに MMIO | 文字列全体をバックバッファ → `flush()` |

バックバッファは `from_info()` で `alloc::vec![0u8; fb_size]` として確保する。ヒープアロケータが先に初期化される（`main.rs` の初期化順序）ので安全。

selftest **35/35 PASS**。

### 追加最適化: ネイティブピクセルフォーマット + 行単位 memcpy

バックバッファを入れても「びみょうに軽くなったかな」程度だったので、さらに踏み込んで最適化した。

**問題**: ユーザー空間の GUI が RGB 固定で描画し、カーネルの `draw_blit_global` が 78 万ピクセルごとに `make_pixel()` で BGR 変換していた。バックバッファのおかげで MMIO への書き込みは速くなったが、変換処理自体の CPU 負荷が残っていた。

**解決策**:
1. **ユーザー空間**: `gui.rs` に `write_pixel` ヘルパーを追加。起動時に `screen_info` syscall から `pixel_format` を取得し、ネイティブフォーマット（BGR）で直接バッファに書き込むようにした
2. **カーネル**: `draw_blit_global` のピクセル単位ループを行単位の `copy_from_slice` に置換。フォーマット変換が不要になったので、78 万回のループ → 768 回の行コピーに削減
3. **flush_rect 最適化**: 全幅かつ `stride == width` の場合に 768 回の行コピー → 1 回の連続 `memcpy` に統合
4. **_print 最適化**: テキスト出力時の flush を全画面 → dirty-rect（変更された行のみ）に変更
5. **draw_rect_global 最適化**: ピクセルループ → 行テンプレート + `copy_within` に変更

| 関数 | Before | After |
|------|--------|-------|
| `draw_blit_global` | 78万回 make_pixel + copy_from_slice | 768回の行 memcpy → 1回の連続 memcpy |
| `flush_rect` (全幅) | 768回の行コピー to MMIO | 1回の連続 memcpy to MMIO |
| `_print` | 全画面 flush (3MB to MMIO) | 変更行のみ flush |
| `draw_rect_global` | put_pixel ×W×H 回 | テンプレート行 + copy_within |
