...
もう1つハマったのが sysroot キャッシュの問題。`make build-user-std` はインクリメンタルビルドで、sysroot のソースを変更しても std の .rlib が再コンパイルされないことがある。`cargo clean` してからビルドし直す必要があった。これは fs のときにも起きた問題で、PAL モジュールを追加するたびに `cargo clean` が必要。面倒だが仕方ない。
