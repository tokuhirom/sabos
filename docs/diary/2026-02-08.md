...
## Makefile 改善: nightly チャンネルの一元管理

### 問題

`rust-toolchain.toml` で `nightly-2026-02-02` に日付固定したのに、以下の3箇所で `nightly` がハードコードされていた:

| 場所 | 問題 |
|------|------|
| `Makefile` の `build-user-std` | `RUSTUP_TOOLCHAIN=nightly` で上書き → 日付固定が無効 |
| `scripts/patch-rust-sysroot.sh` | `rustc +nightly --print sysroot` → 別の sysroot にパッチ適用 |
| `Makefile` の `test` ターゲット | `build-user-std` が依存に含まれていない → HELLOSTD.ELF が古いままテスト |

`nightly` と `nightly-2026-02-02` は別々の sysroot を持つ。パッチを `nightly` の sysroot に当てても、`nightly-2026-02-02` でビルドするとパッチが効かず、std のコンパイルエラーになる。

### 修正

1. **Makefile**: `rust-toolchain.toml` から `NIGHTLY_CHANNEL` 変数を抽出し、`build-user-std` で使用:
   ```makefile
   NIGHTLY_CHANNEL := $(shell grep 'channel' rust-toolchain.toml | sed 's/.*= *"\(.*\)"/\1/')
   # build-user-std:
   cd user-std && RUSTUP_TOOLCHAIN=$(NIGHTLY_CHANNEL) ...
   ```

2. **patch-rust-sysroot.sh**: `+nightly` ハードコードを `rust-toolchain.toml` から読み取る方式に変更

3. **test ターゲット**: `build-user-std` を依存に追加し、HELLOSTD.ELF が常に最新状態でテストされることを保証

4. **HELLOSTD.ELF のテスト順改善**: ネットワークテスト（DNS lookup がタイムアウトする可能性あり）を process/thread/serde テストの後に移動。DNS がハングしても他のテストは先に完了する。

### 学んだこと

- **`nightly` と `nightly-YYYY-MM-DD` は別のツールチェインインストール**。同じ rustc バージョンでも sysroot パスが異なるため、パッチは個別に適用する必要がある。
- **設定の一元管理（Single Source of Truth）が重要**。`rust-toolchain.toml` でバージョンを管理しているのに、Makefile やスクリプトで別の値をハードコードすると整合性が崩れる。全箇所が `rust-toolchain.toml` を参照するようにした。

---

## std::env::args() の修正: 2つのバグを発見・修正

### 背景

HELLOSTD.ELF のテストで `args::count OK: 0` と表示されていた。Phase 9 で `std::env::args()` の仕組みを実装したはずなのに、引数の個数が 0 になっている。QEMU ログで詳細を確認すると:

```
args::count OK: 0
```

argv[0]（プログラム名）すら渡っていない。

### バグ 1: PAL init が argc/argv を無視していた

`rust-std-sabos/sys_pal_sabos_common.rs` の `init()` 関数を見直すと:
...
引数名にアンダースコアがついていて、関数本体は空。せっかくカーネルがレジスタ経由で argc/argv を渡し、`_start_rust` → `main` → `lang_start` → `rt::init` → PAL `init` と伝播してきたのに、ここで捨てていた。
...
`crate::sys::args::init()` は `sys_args_sabos.rs` の Atomic 変数に argc/argv を保存する関数で、Phase 9 で `sys/args/sabos.rs` として作成済みだった。PAL init から呼び出すのを忘れていただけ。

### バグ 2: ページテーブル切替後のダングリング参照

PAL init を修正すると `args::count OK: 1` になった（0 → 1 に改善！）。しかし argv[0] の値が空文字列:

```
args::argv0 OK:         ← 値がない！
```

原因は `kernel/src/syscall.rs` の `exec_by_path_with_args()` にあった:

```rust
fn exec_by_path_with_args(path: &str, extra_args: &[String]) -> Result<(), SyscallError> {
    // ... path はユーザー空間メモリを指す &str ...
    let mut args_vec: Vec<&str> = Vec::with_capacity(1 + extra_args.len());
    args_vec.push(path);  // ← ユーザー空間への参照を保持

    let (current_cr3, current_flags) = Cr3::read();
    unsafe {
        crate::paging::switch_to_kernel_page_table();  // ← ここでページテーブル切替！
    }
    // この時点で args_vec[0] (= path) はカーネルの identity mapping 経由で
    // 別の物理アドレスを読んでしまう！
    let task_id = crate::scheduler::spawn_user(&process_name, &elf_data, &args_vec);
```

`path` はユーザー空間のメモリ（例: 0x400XXX）を指す `&str`。ユーザーのページテーブルでは 0x400XXX → ELF のデータがある物理フレームにマッピングされている。しかし `switch_to_kernel_page_table()` 後は、カーネルの identity mapping（物理アドレス == 仮想アドレス）経由で 0x400XXX = 物理アドレス 0x400XXX を読む。そこには ELF のデータではなく、UEFI やブートコードの残骸（またはゼロ）がある。

つまり `path` はダングリング参照になっている。典型的な use-after-page-table-switch バグ。

修正: カーネルヒープにコピーしてからページテーブルを切り替える。

```rust
let path_owned = String::from(path);  // カーネルヒープにコピー
let mut args_vec: Vec<&str> = Vec::with_capacity(1 + extra_args.len());
args_vec.push(&path_owned);  // ヒープ上の文字列を参照（ページテーブル非依存）
```
...
```
args::count OK: 1
args::argv0 OK: /hellostd.elf
```

正しくプログラム名が argv[0] として渡されるようになった。

### 学んだこと

- **ページテーブル切替は参照の有効性を変える**。Rust の借用チェッカーはメモリの lifetime を検証するが、ページテーブルの切替による仮想アドレスの意味変化までは追跡できない。カーネル開発では「この参照はどのページテーブルで有効か」を常に意識する必要がある。
- **空の init 関数は注意**。PAL の `init` のように「後で実装する」つもりで空にした関数は、忘れがち。特に引数をアンダースコアで受けている場合、コンパイラが警告を出さないので気づきにくい。
...
盛りだくさんな一日だった。朝（前日の続き）から始めて、以下を達成:

1. **SYS_MMAP バグ修正** → `println!` / `String` / `Vec` が動いた
2. **PAL fs 実装** → `std::fs::read_to_string()` / `write()` / `metadata()` が動いた
3. **PAL time 実装** → `Instant::now()` / `elapsed()` が動いた
4. **PAL os/env 実装** → `current_dir()` / `var()` / `set_var()` / `vars()` が動いた
5. **PAL net 実装** → `TcpStream` / `TcpListener` / DNS lookup + UDP ソケットが動いた
6. **PAL process 実装** → `Command::new().spawn()` / `status()` / `wait()` が動いた
7. **PAL thread 実装** → `thread::spawn()` / `join()` / `yield_now()` が動いた
8. **PAL args 実装** → `std::env::args()` が動いた + serde_json が動いた
9. **SystemTime 実装** → CMOS RTC ドライバ + `SYS_CLOCK_REALTIME` syscall
10. **UDP ソケット実装** → netd に UDP プロトコル処理追加 + PAL UdpSocket
11. **IPv6 基盤実装** → ICMPv6 Echo + NDP で `ping6` が動作
12. **スラブアロケータ実装** → O(1) alloc/dealloc + Large 領域の結合・巻き戻し
13. **TODO_std.md 残タスク消化** → OOM 問題解決済みマーク + nightly 日付固定
14. **Makefile 改善** → nightly チャンネル一元管理 + test 依存修正
15. **std::env::args() バグ修正** → PAL init の argc/argv 転送 + ダングリング参照修正

Phase 7（std 基盤）→ Phase 8（PAL 充実）→ Phase 9（args + 外部クレート）→ Phase 10（UDP + IPv6 + スラブ）→ TODO 消化 → バグ修正 と、怒涛の進捗。

### 特に面白かったこと

- **ダングリング参照バグの発見**。Rust の借用チェッカーが通っているのにページテーブル切替でメモリの意味が変わる。カーネル開発ならではのバグで、「Rust の安全性の境界はどこにあるのか」を体感できた。
- **スラブアロケータ**。教科書で読んだ概念を実装して、実際に動くのを見ると理解の深さが全然違う。特に intrusive linked list のエレガントさ（解放済みメモリ自体をリンクノードに使う）に感動。
- **PAL 追加の繰り返し**。fs → time → env → net → process → thread → args と、同じパターン（PAL ファイル作成 → パッチスクリプト更新 → テスト追加）を何度も繰り返すうちに完全に手に馴染んだ。

### 開発サイクルの改善

今日の作業で改善した開発体験:

1. **sysroot キャッシュ自動検出** — PAL パッチ変更時の `cargo clean` を自動化。手動 clean 忘れによるデバッグ時間ロスを撲滅
2. **syscall 番号一元管理** — `libs/sabos-syscall/` クレート + CI 検証スクリプトで番号ドリフトを構造的に防止
3. **nightly チャンネル一元管理** — `rust-toolchain.toml` から全箇所が参照する Single Source of Truth
4. **QEMU ログ保存機能** — `SABOS_SAVE_LOG` 環境変数で QEMU の全出力を保存。テスト結果の詳細分析が容易に
5. **テスト順序の最適化** — ネットワークテスト（タイムアウトの可能性あり）を最後に配置

### 残っている課題

- **HELLOSTD.ELF のネットワークテスト**: `net::lookup` と `net::udp_bind` が IPC タイムアウトで失敗する。netd との通信がうまくいっていない。原因は調査中。
- **カーネルパニック**: HELLOSTD.ELF のネットワークテスト後に INVALID OPCODE (#UD) でカーネルパニックが起きている。ネットワークのタイムアウト処理にバグがある可能性。
- **grep コマンドのテスト**: シェルの `grep` コマンドのテストが期待通りの出力を返していない（WARN）。
