...
- **シェルの PATH 機能は Unix の基本だけど改めて便利**: `run /9p/x86_64-unknown-none/debug/tetris` と毎回長いパスを打つのは非現実的。検索パスを導入して `tetris` だけで動くようにしたら体験が一変した。POSIX 互換は目指さない方針だけど、良い UX のアイデアは積極的に取り入れるべき

### ユーザーシェルに PATH 機能を追加

virtio-9p でホストのバイナリにアクセスできるようになったが、実行するのに `run /9p/x86_64-unknown-none/debug/tetris` と打つのは長すぎる。Unix シェルの PATH 環境変数のように、コマンド名だけで実行ファイルを自動検索する機能を追加した。

#### 仕組み

ビルトインコマンド（echo, ls, cat 等）に該当しない入力があると、以下のディレクトリを順に探索する:

1. `/9p/x86_64-unknown-none/debug/` — virtio-9p 経由のビルド成果物（最優先）
2. `/host/` — hostfs.img 上の ELF バイナリ
3. `/` — ルート FAT32 上の ELF バイナリ

スラッシュを含む入力（`./foo` や `/path/to/bin`）は直接パスとして扱う。引数も `exec_with_args` 経由で渡せる。

#### 使用例

```
user> tetris          # /9p/x86_64-unknown-none/debug/tetris を実行
user> calc            # /9p/x86_64-unknown-none/debug/calc を実行
user> HELLO.ELF       # / から探して /HELLO.ELF を実行
user> help            # 検索パスの一覧も表示される
```

`cargo build` → `tetris` の 2 ステップで最新のゲームが遊べる。開発体験がさらに良くなった。
