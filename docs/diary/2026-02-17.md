...
ネットワーク移行後、最初のテストでは telnetd 経由のユーザーランドテストが全て失敗した。カーネル selftest は 50/50 PASS なのに、telnet で接続してコマンドを送ると応答が返ってこない。

原因を調べたところ、httpd と telnetd が TCP accept を食い合っていた。両方のサービスがそれぞれ `tcp_listen` + `tcp_accept` を呼ぶが、カーネル内のネットワークスタックでは accept のタイミングで他のサービス宛のパケットも処理してしまう。netd 時代は IPC で各サービスが独立していたので問題にならなかったが、カーネル内に統合したことでこの競合が顕在化した。

対策として、httpd をデフォルト起動のサービスから外した。init.rs の SERVICES 配列を 5→4 に縮小。httpd が必要な場合はシェルから `spawn /HTTPD.ELF` で手動起動する。

これで telnetd が TCP ポート 2323 の accept を独占でき、ユーザーランドテストも全て PASS するようになった。

httpd_service テスト（HTTPD.ELF タスクの存在確認）は削除し、httpd_dirlist テストは実態に合わせて vfs_dirlist に改名した（このテストは httpd と無関係に VFS のディレクトリ一覧機能をテストしていた）。

## テスト結果

カーネル selftest: **49/49 全 PASS**
ユーザーランドテスト（telnet 経由）: **全 PASS**

```
[PASS] network_dns      ← カーネル内 netstack::dns_lookup() 直接呼び出し
[PASS] telnetd_service   ← telnetd 動作確認
[PASS] vfs_dirlist        ← 旧 httpd_dirlist を改名
```

## 差分サマリ

```
19 files changed, 859 insertions(+), 3667 deletions(-)  （ネットワーク移行）
2 files changed, 10 insertions(+), 22 deletions(-)      （httpd 停止）
```
...
- **TCP accept の競合**: 複数サービスが同じネットワークスタックで `tcp_accept` を呼ぶと、ポーリング中に他サービス宛のパケットを処理してしまう。根本的にはポートごとに accept キューを分離する必要がある（今回は httpd を止めることで回避）

## 振り返り

移行は8ステップに分割して計画的に進めた。特にステップ1（カーネル netstack 作成）が最大の作業量だったが、ユーザー空間版をベースにしたため、ロジックの再実装は不要で、主にロック戦略の変更に集中できた。

移行直後のテストで telnetd が壊れたのは焦ったが、httpd との accept 競合という原因が特定できた。カーネル内統合で IPC のオーバーヘッドは消えたが、代わりに「複数サービスがカーネルリソースを共有する」という新しい問題が出てきた。将来的にはポートごとの accept キュー分離を実装すれば httpd も同時起動できるようになるはず。全テスト PASS で気持ちよく一日を終われた。
