...
## Capability ベースの実装を日常運用に — シェルをハンドル API に移行

TODO.md の「Capability ベースの実装を日常運用に」タスクの一環として、シェルの 4 コマンド（write, rm, mkdir, rmdir）をパスベース syscall からハンドルベース API に移行した。

### 背景

これまで ls, cat, grep, sed, cd などの読み取り系コマンドはハンドルベース API に移行済みだったが、書き込み系の 4 コマンドはまだパスベースの `file_write`, `file_delete`, `dir_create`, `dir_remove` を使っていた。ハンドルの権限ビット `CREATE`(0x0020) と `DELETE`(0x0040) は定義されていたが、それを活用する syscall が存在しなかった。
...
| 番号 | 名前 | 動作 | 必要な権限 |
|------|------|------|-----------|
| 140 | `SYS_HANDLE_CREATE_FILE` | ディレクトリ内にファイルを作成し、RW ハンドルを返す | CREATE |
| 141 | `SYS_HANDLE_UNLINK` | ディレクトリ内のファイル/空ディレクトリを削除 | DELETE |
| 142 | `SYS_HANDLE_MKDIR` | ディレクトリ内にサブディレクトリを作成 | CREATE |

これらはすべてディレクトリハンドルを起点とする。ファイル名に "/", "..", "." を含むパストラバーサル攻撃を構造的にブロックする `validate_entry_name()` ヘルパーを追加した。

**シェルコマンドの書き換え:**

各コマンドの流れ:
- `write FILE TEXT` → `resolve_dir_and_name()` で親ディレクトリと名前を分離 → `handle_create_file()` でファイル作成 → `handle_write()` でデータ書き込み → `handle_close()` でフラッシュ
- `rm FILE` → 同じく親を解決 → `handle_unlink()` で削除
- `mkdir DIR` → 親を解決 → `handle_mkdir()` で作成
- `rmdir DIR` → 親を解決 → `handle_unlink()` で削除（ファイル/ディレクトリ両方に対応）

`resolve_dir_and_name()` は `rfind('/')` でパスの最後のスラッシュを見つけ、親ディレクトリ部分とファイル名を分離する。`/SUBDIR/FILE` なら `/SUBDIR` を open して `FILE` を返し、`FILE` だけなら cwd_handle をそのまま使う。

**権限の修正:**

重要な修正として、cwd_handle（カレントディレクトリのハンドル）をフルアクセス権限（`HANDLE_RIGHTS_DIRECTORY` = STAT|ENUM|CREATE|DELETE|LOOKUP）で開くように変更した。以前は READ-ONLY（`HANDLE_RIGHTS_DIRECTORY_READ`）だったため、cwd 内でのファイル作成/削除が権限不足でエラーになっていた。

また、`openat` がデフォルトで rights=0（READ-ONLY）でハンドルを作成していたのを、親ハンドルの権限を引き継ぐように修正した。これは Capability-based security の原則（「親の権限以下で子を開く」）に沿った変更。

### テスト

3 つの新しい selftest を追加:
- `handle_create_file`: ファイル作成 → 書き込み → 読み返し → 内容確認 → クリーンアップ
- `handle_unlink`: ファイル作成 → 削除 → 存在しないことを確認
- `handle_mkdir`: ディレクトリ作成 → 存在確認 → クリーンアップ

全 45 テスト中 44 PASS（`network_netd_dns` のみ既知の FAIL）。

### 開発サイクルの振り返り

**ビルドエラーで気づいた点:**
- `kernel/src/shell.rs` 内のテスト関数で素の `format!()` を使ったらビルドエラーになった。`syscall.rs` では `use alloc::format;` をインポートしていたが、`shell.rs` にはなかった。対処として `format!()` が不要なコード（定数文字列リテラル）に書き換えた。テスト用のパスは `"/HCFTEST.TXT"` のように定数にする方が `format!("/{}", name)` よりも効率的で良い

**パスベース syscall は削除できるか?**
- 調査したところ、`rust-std-sabos/` の PAL（`std::fs::remove_file`, `std::fs::write` 等）がまだパスベース syscall に依存している
- ユーザーシェルからは使われなくなったが、std ライブラリ経由の利用があるため削除不可
- 将来的に PAL もハンドルベースに移行したい（TODO として記録）

**openat の権限引き継ぎ修正:**
- 今回 `openat` が親ハンドルの権限を引き継ぐように修正したが、これは本来 Capability-based security の基本原則なので最初から実装すべきだった。新しい syscall を追加するときは、権限の流れ（誰から何を引き継ぐか）を最初に設計すること

## ネットワーク selftest 安定化

### 問題

ネットワーク selftest の DNS クエリが常に失敗していた。IPv4 の DNS クエリを送信してもレスポンスが一切返ってこない状態。pcap でキャプチャしたところ、DNS クエリは正しい形式で送信されているが、QEMU の SLIRP バックエンドがレスポンスを返さないことが判明。

### 根本原因の発見

QEMU 8.2 の `-netdev user,...,ipv6=on` オプションが **暗黙的に IPv4 を無効にする** ことが原因だった。

```
# エラーになる！
qemu-system-x86_64 -netdev user,id=n0,ipv6=on,dns=8.8.8.8
→ "IPv4 disabled but netmask/host/dns provided"
```

つまり `ipv6=on` だけを指定すると、IPv4 ネットワーキング全体が無効になる。DNS, TCP, UDP すべてが使えなくなっていた。

### 発見の経緯

1. QEMU の `filter-dump` で pcap キャプチャ → DNS クエリは送信されているがレスポンスなし
2. UDP チェックサムが 0 だったので計算を追加 → それでも失敗（根本原因ではなかった）
3. SLIRP の DNS オプションを変更しようと `dns=192.168.1.1` を追加 → **"IPv4 disabled"** のエラーメッセージで原因が判明
4. `ipv4=on,ipv6=on` に変更 → 即座に全テスト PASS

### 修正

**Makefile** と **scripts/run-selftest.sh** の QEMU ネットワークオプションを変更:
...
### QEMU の `ipv6=on` の罠（ソースコード調査）

なぜ `ipv6=on` だけで IPv4 が無効になるのか不思議だったので、QEMU のソースコード（[net/slirp.c](https://github.com/qemu/qemu/blob/master/net/slirp.c)）を調べた。`net_init_slirp` 関数に以下のロジックがある:

```c
bool ipv4 = true, ipv6 = true;

// ipv6=on を指定 && ipv4 を指定していない → ipv4 を無効化
if ((user->has_ipv6 && user->ipv6 && !user->has_ipv4) ||
    (user->has_ipv4 && !user->ipv4)) {
    ipv4 = 0;
}
// ipv4=on を指定 && ipv6 を指定していない → ipv6 を無効化
...
つまり「片方だけ明示的に on にした = もう片方は要らない」という設計。ドキュメントの "Specify that either IPv4 or IPv6 must be enabled" はこの排他的な動作を指している。`ipv6=on` は「IPv6 を有効にする」ではなく「IPv6 **だけ** を有効にする」という意味だった。デュアルスタックが必要なら `ipv4=on,ipv6=on` と両方明示する必要がある。

何もオプションを指定しなければ `ipv4=true, ipv6=true` のデフォルトでデュアルスタックになるので、IPv6 を追加したいだけなら実はオプション不要だった。ただし IPv6 のネットワークアドレス指定（`ipv6-net` 等）を使いたい場合は明示が必要。

### 副産物の改善

デバッグ中に発見した問題も合わせて修正:

1. **UDP チェックサム計算の追加** (`kernel/src/net.rs`, `user/src/netstack.rs`)
   - IPv4 では UDP チェックサムはオプションだが、正しく計算するよう修正
   - 疑似ヘッダーを含む RFC 準拠の実装

2. **ビジーループの排除** (`user/src/netstack.rs`)
   - `spin_loop()` によるビジーウェイトを `poll_and_handle_timeout()` に置換
   - 100ms ブロッキング受信で CPU 使用率を大幅削減
   - dns_lookup, tcp_connect, tcp_recv, tcp_accept, udp_recv_from, tcp_close, wait_icmpv6_echo_reply

3. **QEMU SLIRP 互換性の改善** (`kernel/src/syscall.rs`)
   - `sys_net_recv_frame` のタイムアウト付きループで ISR ステータス読み取り + `enable_and_hlt()` を使用
   - QEMU TCG モードではゲスト CPU がビジーループすると SLIRP の I/O が処理されない問題を回避

4. **デバッグ出力のクリーンアップ**
   - 前回セッションで追加したデバッグ用 `serial_println!` と `write_str()` を全て削除
   - `netd_is_running()` デッドコード削除

### テスト結果

- ネットワーク API selftest: 5/6 PASS（`net_ipv6_ping` のみ FAIL — IPv6 は SLIRP の制約で未対応）
- カーネル selftest: 42/45 PASS（`ipc`, `network_netd_dns`, `gui_ipc` は既知の FAIL）
