...
## HELLOSTD.ELF net テストの安定化

リファクタリングと IPv6 テストの安定化が終わったので、もう一つのフレーキーテスト問題に取り組んだ。HELLOSTD.ELF（std 対応バイナリ）のネットワークテストが CI で時々タイムアウトする問題だ。

### 問題の整理

HELLOSTD.ELF は Rust の `std::net` をテストするために DNS lookup と UDP 通信を行っている。しかし QEMU の SLIRP バックエンドは応答が遅いことがあり、特に CI 環境で UDP recv がタイムアウトすることがあった。

もう一つ気づいたのは、ソースコードのコメントに「DNS lookup が netd との IPC タイムアウトで…」と書いてあったが、既にカーネル syscall 直接呼び出しに移行済みだったこと。コメントが実装に追いついていなかった。

### 変更内容

3つの変更を行った:

1. **UDP recv タイムアウトを 5秒→10秒に延長**（`user-std/src/main.rs`）
   SLIRP の応答遅延に余裕を持たせた。DNS lookup はカーネル側で 5秒×2リトライなので変更不要。

2. **テストスクリプトに `net::tcp_parse OK` の検証を追加**（`scripts/run-selftest.sh`）
   `net::tcp_parse` はアドレスのパーステスト（`"10.0.2.2:80".parse()`）なのでネットワーク通信不要で常に成功する。これをスクリプトの検証対象に加えることで、ネットワーク部分のコードが少なくとも実行されていることを確認できる。DNS/UDP の通信結果は SLIRP 依存で flaky なのでスクリプトでは検証しない方針にした。

3. **コメントの更新**
   「netd との IPC タイムアウト」→「カーネル syscall のタイムアウト」に修正。

### expect の出力順序ハマり

最初にテストを走らせたら HELLOSTD のテストが FAIL した。原因は expect スクリプトの検証順序ミス。main.rs では serde テストが先（227行目）、net テストが後（254行目）なので、出力は `serde::from_str OK` → `net::tcp_parse OK` の順になる。expect で `net::tcp_parse OK` を先にマッチしようとしたので見つからなかった。順序を正しく `serde::from_str OK` → `net::tcp_parse OK` に直して解決。expect はバッファを消費しながら順にマッチするので、出力順序と一致させないといけない。

### TODO.md の更新

ついでに TODO.md も整理した:
- AC97 の「実際に音を鳴らす」タスクを完了マーク（`play_tone` + `beep` コマンドは既に実装済みだった）
- HELLOSTD net テストのフレーキーさ改善タスクを完了マークし、対応内容を記録

65/65 selftest PASS + ユーザーランドテスト全 PASS を確認してコミット・プッシュ。
