...
x86_64 のページテーブルは 4 階層の木構造になっていて、それぞれ正式名称がある。コード中では階層番号で L4/L3/L2/L1 と呼んでいる（`x86_64` crate のドキュメントや OS 開発チュートリアルでも一般的な呼び方）:

| 略称 | 正式名称 | 略語の由来 | 管理単位 |
|------|----------|-----------|---------|
| L4 | PML4 (Page Map Level 4) | そのまま「4階層目のページマップ」 | 512GiB |
| L3 | PDPT (Page Directory Pointer Table) | ページディレクトリを指すポインタのテーブル | 1GiB |
| L2 | PD (Page Directory) | ページテーブルを束ねるディレクトリ | 2MiB |
| L1 | PT (Page Table) | 最終段のページテーブル。物理フレームを直接指す | 4KiB |

名前がややこしいのは歴史的経緯のせい。もともと x86 のページングは 2 階層（PD → PT）だけだったのが、メモリ空間が広がるたびに上に階層が追加されていった。後から足された階層ほど名前が長くなっている。L4 の PML4 は x86_64 (IA-32e) で導入された最上位の階層。

仮想アドレスの各ビット範囲がそれぞれの階層のインデックスに対応している:

```
仮想アドレス (48ビット):
[47:39] → L4 (PML4) のインデックス  (512 エントリ × 512GiB = 256TiB)
[38:30] → L3 (PDPT) のインデックス  (512 エントリ × 1GiB = 512GiB)
[29:21] → L2 (PD)   のインデックス  (512 エントリ × 2MiB = 1GiB)
[20:12] → L1 (PT)   のインデックス  (512 エントリ × 4KiB = 2MiB)
[11:0]  → ページ内オフセット (4KiB)
```

CPU はメモリアクセスのたびにこの 4 階層を辿って物理アドレスを求める（実際には TLB キャッシュがあるので毎回辿るわけではない）。

#### USER_ACCESSIBLE (U/S ビット) とは

ページテーブルエントリのビット 2。このビットが 0 のページに Ring 3 からアクセスすると、CPU が自動的に Page Fault (#PF) を発生させる。USER_ACCESSIBLE は全階層 (L4/L3/L2/L1) に設定が必要。どれか 1 箇所でも欠けると Ring 3 からアクセスできない。
...
### Task 7: プロセス分離 — プロセスごとのページテーブル ✅

Task 6 では共有ページテーブルの USER_ACCESSIBLE フラグを切り替える方式でメモリ保護を実現した。でもこの方式には根本的な弱点がある。全プロセスが同じページテーブルを共有しているので、「プロセス A のメモリをプロセス B から隠す」ことができない。本物の OS ではプロセスごとに独立したアドレス空間を持っていて、あるプロセスが別プロセスのメモリを直接読み書きすることはできない。

今回これを実装した。プロセスごとに専用の L4 ページテーブルを持たせ、CR3 レジスタの切り替えでアドレス空間を丸ごと分離する方式に改善した。

#### CR3 レジスタとアドレス空間の切り替え

CR3 レジスタは L4 ページテーブルの物理アドレスを保持する特殊なレジスタ。CPU はメモリアクセスのたびに CR3 が指すページテーブルを参照してアドレス変換を行う。つまり CR3 を書き換えるだけで、アドレス空間が丸ごと切り替わる。しかも CR3 への書き込みは TLB（ページテーブルのキャッシュ）を自動的にフラッシュしてくれるので、古いマッピング情報が残る心配もない。

プロセス A が CR3=0x104000、プロセス B が CR3=0x108000 なら、それぞれまったく異なるアドレス変換テーブルを使っている。同じ仮想アドレスでも異なる物理ページにマッピングされるし、USER_ACCESSIBLE の設定もプロセスごとに独立している。

#### L4 エントリの共有と分岐コピー

新しい L4 テーブルを作るとき、カーネルの L4 エントリをすべてコピーする。L4 エントリは L3 テーブルの物理アドレスを指すポインタなので、コピーするだけで L3 以下の木構造全体を共有できる。カーネルのコード・データはすべての L4[0]（0〜512GiB）の範囲に収まっているので、L3 以下のテーブルを複製する必要はない。

ただし USER_ACCESSIBLE をプロセス固有に設定するには、カーネルと共有している中間テーブルを「分岐コピー (fork copy)」する必要がある。例えばユーザーコードのアドレス `0x06200000` のページに USER_ACCESSIBLE を設定したい場合:

1. プロセスの L4[0] がカーネルの L3 テーブルを指している（共有状態）
2. 新しい L3 テーブルをフレームアロケータから確保し、カーネル L3 の内容をコピー
3. プロセスの L4[0] を新しい L3 に差し替え
4. 同様に L2、L1 も必要に応じてコピー
5. コピーした L1 エントリに USER_ACCESSIBLE を追加

この「共有しているなら分岐コピー、そうでなければそのまま使う」というパターンは、Unix の fork + copy-on-write に似た考え方。ただし今回はページの内容ではなく、ページテーブル自体の分岐コピー。

#### UserProcess 構造体

```rust
pub struct UserProcess {
    pub page_table_frame: PhysFrame<Size4KiB>,  // L4 ページテーブルのフレーム
    pub kernel_stack: Box<[u8]>,                 // プロセス固有のカーネルスタック
}
```

`create_user_process()` でページテーブル作成 + USER_ACCESSIBLE 設定 + カーネルスタック確保をまとめて行い、`run_in_usermode()` は CR3 切り替え → Ring 3 遷移 → 復帰後に CR3 をカーネルに戻す、というシンプルな流れになった。旧方式の `set_user_accessible()` / `clear_user_accessible()` は呼ばなくなった。

#### フレームの解放

プロセスを破棄するとき、分岐コピーで作ったテーブル（L3/L2/L1）を正しく解放する必要がある。カーネルと共有しているテーブルは解放してはいけない（他のプロセスやカーネル自身が使っている）。判定方法は単純で、プロセスの各エントリがカーネルの対応するエントリと同じアドレスを指しているかどうかを比較する。異なるアドレスならプロセス固有のコピーなので解放する。

実際に `isolate` コマンドで確認したところ、プロセス A/B を作成して破棄した後に 8 フレームが回収された。各プロセスで L4 + L3 + L2 + L1 の 4 フレーム × 2 プロセス = 8 フレーム。期待通り。

#### 結果

`isolate` コマンドで 2 つのプロセスを別々のページテーブルで実行:

```
sabos> isolate
=== Process Isolation Demo ===
Kernel CR3: 0x7801000

Process A: CR3 = 0x104000
  Running...
Hello from Ring 3!
  Done!
Process B: CR3 = 0x108000
  Running...
Hello from Ring 3!
  Done!

Result: CR3 A (0x104000) != CR3 B (0x108000)
  => Each process has its own page table!
  => Address spaces are ISOLATED.
Free frames before cleanup: 20953
Free frames after cleanup:  20961
Frames reclaimed: 8
=== Demo Complete ===
```

CR3 が異なる = ページテーブルが別物 = アドレス空間が分離されている。既存の `usermode` コマンドも `usertest` コマンドも新方式で正常に動作。

![プロセス分離デモ](images/isolate.png)

## 次にやりたいこと

- **プロセスごとのユーザースタック**: 現在は全プロセスで静的な `USER_STACK` を共有している。プロセスごとに動的にスタックを割り当てれば、複数プロセスの同時実行に対応できる
- **ELF バイナリのロード**: カーネルバイナリに埋め込んだ関数ではなく、外部の ELF バイナリをメモリにロードして Ring 3 で実行する
- **ファイルシステム**: FAT32 か簡易的なファイルシステムを実装して、ディスクからプログラムを読み込めるようにする
- **仮想アドレス空間のレイアウト設計**: カーネル空間とユーザー空間を上位/下位で分離する（現在はアイデンティティマッピングの範囲にすべて載せている）
