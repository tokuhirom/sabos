// sabos-fat32 — FAT32 ファイルシステム ジェネリックライブラリ
//
// BlockDevice トレイトに対してジェネリックな FAT32 ドライバ。
// カーネル内 (KernelBlockDevice) でもユーザー空間 (UserBlockDevice) でも使える。
//
// kernel/src/fat32.rs のコアロジックを分離したもの。
// VFS 統合や OS 固有の部分はカーネル側に残る。
...
use sabos_blockdev::BlockDevice;
use sabos_fat_core::{
...
///
/// ジェネリックパラメータ D により、カーネル内でもユーザー空間でも使える。
/// - カーネル: `Fat32Fs<KernelBlockDevice>` — virtio-blk を直接操作
/// - ユーザー: `Fat32Fs<UserBlockDevice>` — syscall 経由でブロック I/O
pub struct Fat32Fs<D: BlockDevice> {
    ...
    /// ブロックデバイス。カーネル側で dev_index を参照するため pub にしている。
    pub dev: D,
}

impl<D: BlockDevice> Fat32Fs<D> {
    /// BlockDevice を渡して Fat32 を初期化する
...
pub fn split_parent(path: &str) -> Result<(&str, &str), &'static str> {
    let path = path.trim();
