...
- [ ] TCP 初期シーケンス番号（ISN）をランダム化（`netstack.rs:487` の `let initial_seq = 1000;`）
  - カーネルの乱数生成器（RDRAND / TSC ベース）を使う
- [ ] DNS クエリ ID をランダム化（`netstack.rs:999` の `0x1234`）
- [ ] DNS ソースポートをエフェメラルポートからランダム選択（`netstack.rs:1000` の `12345`）
